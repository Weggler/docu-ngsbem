 ...
 //    }
    {
      ...
      LegendreFunctions legendre_m1(1);
      LocalHeap lh(( 32*500));
      FlatVector<double> legendre_values(lmax + 1, lh);
      legendre_m1.Eval(lmax, ctheta, legendre_values);
      
      for (l = 0; l < lmax; l++) {
        double pml1 = legendre_values(l);
        double pml;
        if (l == 0)
          pml = 0.;
        else
          pml = legendre_values(l-1);
          // double pml = legendre_values(l);
          plk[l] = pml1;
          dplk[l] = ((2. + l) * pml - (1. + l) * ctheta * pml1) / ((1. + ctheta) * (1. - ctheta));
      }
      p  =  plk[0];
  ...
  // n == 1
  // 
  LocalHeap lh(( 132*500));
  FlatVector<double> j_sph(lmax + 1, lh), y_sph(lmax + 1, lh);
  FlatVector<double> jp_sph(lmax + 1, lh), yp_sph(lmax + 1, lh);
  ngsbem::SBESJY(x, lmax, j_sph, y_sph, jp_sph, yp_sph);
  // rbessel = std::cyl_bessel_j(1.5, x);
  // ibessel = std::cyl_neumann(1.5, x);
  double sqrt_factor = sqrt(M_PI / (2.0 * x));
  for (int l = 0; l <= lmax; l++)
  {
    j_sph(l) /= sqrt_factor;
    y_sph(l) /= sqrt_factor;
    jp_sph(l) /= sqrt_factor;
    yp_sph(l) /= sqrt_factor;
  }
  rbessel = j_sph(1);
  ibessel = y_sph(1);
  jlk[0]  = rbessel;
