...
def run_laplace_ntd(order, i):
    ...
    start = time.time()
    with TaskManager():
        pre = BilinearForm(uH1*vH1*ds(bonus_intorder=intorder)).Assemble().mat.Inverse(freedofs=fesH1.FreeDofs())
        ...
        CG(mat=D.mat+S, pre=pre, rhs=rhs, sol=u0.vec, tol=1e-8, maxsteps=200, initialize=False, printrates=False)
    ...
    errd = sqrt(Integrate( (grad(u0exa)-grad(u0))**2, mesh.Boundaries(".*"), BND))
...
def main():
    ...
    for order in range(1, 5):
        for i in range(1, 7):
            results.append(run_laplace_ntd(order, i))
